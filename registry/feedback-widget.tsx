...
export function FeedbackWidget({
  ...
  const close = useCallback(() => {
    clearCloseTimer();
    restoreFocusRef.current = !!rootRef.current?.contains(
  ...
  return (
    <div
      ref={rootRef}
      className={cn(
        "pointer-events-none absolute bottom-4 z-30 w-[calc(100%_-_2rem)]",
        left ? "left-4" : "right-4",
        className,
      )}
    >
      ...
      <motion.div
        ...
        className={cn(
          "pointer-events-auto absolute bottom-0 overflow-hidden border border-[#333333] bg-[#171717] text-[#f2f2f2] shadow-lg",
          open ? "w-full max-w-80 p-2" : "h-12 w-12 p-0",
          left ? "left-0" : "right-0",
        )}
      >
        <AnimatePresence initial={false} mode="popLayout">
          {open ? (
                ...
                <AnimatePresence mode="popLayout" initial={false} propagate>
                  {status === "sent" ? (
                    <motion.div
                      ...
                      exit={viewExit}
                    >
                      <div className="flex flex-col items-center justify-center gap-1.5 rounded-[20px] bg-[#262626]/60 px-4 py-6 text-center">
                    ...
                    >
                      <div
                        role="alert"
                        className="rounded-[20px] bg-[#262626]/60 px-4 py-5 text-center"
                      >
                        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-500/10 text-red-400">
                          <AlertCircle className="h-5 w-5" />
                        </div>
                        <h3 className="mt-3 text-sm font-semibold text-[#f2f2f2]">
                    ...
                    <motion.div
                      ...
                      exit={viewExit}
                    >
                      <div className="min-h-[150px] rounded-[20px] bg-[#262626]/60 px-4 py-3.5">
                        <div className="flex items-start justify-between gap-3">
                          <h3
                            id={titleId}
                            className="text-sm font-semibold text-[#f2f2f2]"
                          >
                            {title}
                          </h3>
                          <button
                            ...
                            disabled={busy}
                            className="-mr-2 -mt-2 flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white/10 text-neutral-400 transition-colors hover:text-[#f2f2f2]"
                          >
                            <X className="h-3 w-3" />
                          </button>
                        </div>
                        ...
                        <textarea
                          ...
                          disabled={busy}
                          rows={3}
                          className="mt-2 w-full resize-none bg-transparent text-base text-[#f2f2f2] rounded-sm focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-neutral-400 placeholder:text-neutral-400 md:text-sm"
                        />
                      </div>
                      ...
                      <div className="flex items-center gap-2 px-1 pt-2 pb-1">
                        <Button
                          ...
                          onClick={close}
                          disabled={busy}
                          className="flex-1 bg-[#262626] text-neutral-400 hover:text-[#f2f2f2]"
                        >
                          Cancel
                        </Button>
                        <Button
                          ...
                          className="flex-1 bg-[#f2f2f2] text-[#171717] hover:bg-white/90"
                        >
                          {busy ? "Sending…" : "Submit"}
                        </Button>
                      </div>
            ...
            <motion.button
              ...
              className={cn(
                "focus-visible:outline-2 focus-visible:outline-offset-[-4px] focus-visible:outline-neutral-400 absolute bottom-0 flex h-12 w-12 items-center justify-center bg-transparent text-[#f2f2f2]",
                left ? "left-0" : "right-0",
              )}
            >
              <motion.span
