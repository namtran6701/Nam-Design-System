...
export type HookSidebarProps = Omit<ComponentProps<"nav">, "onChange"> & {
  /** Desktop navigation; hidden below Tailwind lg (1024px). */
  items: HookSidebarItem[];
  /** Pass your router pathname or active hash when using link items. */
  currentHref?: string;
  label?: string;
  value?: number;
  defaultValue?: number;
...
export function HookSidebar({
  ...
  const routed = currentHref !== undefined;
  const routeIndex = items.findIndex((item) => hrefOf(item) === currentHref);
  ...
  return (
    <nav
      ...
      aria-label={label}
      className={cn("hidden flex-col text-neutral-900 lg:flex", className)}
      {...props}
    >
      {label && (
        <span
          data-slot="hook-sidebar-label"
          className="pb-3 pl-0.5 pr-2 font-sans text-sm font-medium uppercase tracking-wide text-neutral-900"
        >
          {label}
        </span>
      )}
      ...
      >
        ...
        {items.map((item, index) => {
          ...
          const rowProps = {
            ...
            className: cn(
              "rounded-lg py-1.5 pl-5 pr-2 text-left text-sm transition-colors duration-200 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-current motion-reduce:transition-none",
              isActive
                ? "text-neutral-900"
